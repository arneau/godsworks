...
/**
 * Base class that represents a row from the 'passage' table.
 *
 *
 *
* @package    propel.generator..Base
*/
abstract class Passage implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\PassageTableMap';
    ...
    /**
     * Initializes internal state of Base\Passage object.
     */
    ...
    /**
     * Compares this with another <code>Passage</code> instance.  If
     * <code>obj</code> is an instance of <code>Passage</code>, delegates to
     * <code>equals(Passage)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Passage The current object, for fluid interface
     */
    ...
    public function setBibleId($v)
    {
        ...
        if ($this->bible_id !== $v) {
            $this->bible_id = $v;
            $this->modifiedColumns[PassageTableMap::COL_BIBLE_ID] = true;
        }
    ...
    public function setText($v)
    {
        ...
        if ($this->text !== $v) {
            $this->text = $v;
            $this->modifiedColumns[PassageTableMap::COL_TEXT] = true;
        }
    ...
    public function setVerseId($v)
    {
        ...
        if ($this->verse_id !== $v) {
            $this->verse_id = $v;
            $this->modifiedColumns[PassageTableMap::COL_VERSE_ID] = true;
        }
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[PassageTableMap::COL_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : PassageTableMap::translateFieldName('BibleId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->bible_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PassageTableMap::translateFieldName('Text', TableMap::TYPE_PHPNAME, $indexType)];
            $this->text = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PassageTableMap::translateFieldName('VerseId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->verse_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : PassageTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = PassageTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Passage'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildPassageQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see Passage::setDeleted()
     * @see Passage::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildPassageQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                PassageTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PassageTableMap::COL_BIBLE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'bible_id';
        }
        if ($this->isColumnModified(PassageTableMap::COL_TEXT)) {
            $modifiedColumns[':p' . $index++]  = 'text';
        }
        if ($this->isColumnModified(PassageTableMap::COL_VERSE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'verse_id';
        }
        if ($this->isColumnModified(PassageTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }

        $sql = sprintf(
            'INSERT INTO passage (%s) VALUES (%s)',
    ...
    {
        $pos = PassageTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Passage'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Passage'][$this->hashCode()] = true;
        $keys = PassageTableMap::getFieldNames($keyType);
        $keys_verse = \Map\VerseTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getBibleId(),
            $keys[1] => $this->getText(),
            $keys[2] => $this->getVerseId(),
            $keys[3] => $this->getId(),
            $keys_verse[0] => $this->getBookId(),
            $keys_verse[1] => $this->getChapterNumber(),
            $keys_verse[2] => $this->getVerseNumber(),
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PassageTableMap::COL_BIBLE_ID)) {
            $criteria->add(PassageTableMap::COL_BIBLE_ID, $this->bible_id);
        }
        if ($this->isColumnModified(PassageTableMap::COL_TEXT)) {
            $criteria->add(PassageTableMap::COL_TEXT, $this->text);
        }
        if ($this->isColumnModified(PassageTableMap::COL_VERSE_ID)) {
            $criteria->add(PassageTableMap::COL_VERSE_ID, $this->verse_id);
        }
        if ($this->isColumnModified(PassageTableMap::COL_ID)) {
            $criteria->add(PassageTableMap::COL_ID, $this->id);
        }
    ...
    {
        $criteria = ChildPassageQuery::create();
        $criteria->add(PassageTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Passage (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Passage Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if (null !== $this->aBible) {
            $this->aBible->removePassage($this);
        }
        if (null !== $this->aVerse) {
            $this->aVerse->removePassage($this);
        }
        $this->bible_id = null;
        $this->text = null;
        $this->verse_id = null;
    ...
    {
        return (string) $this->exportTo(PassageTableMap::DEFAULT_STRING_FORMAT);
    }
    ...
    if (is_callable(array('\Verse', $name))) {
        if (!$delegate = $this->getVerse()) {
            $delegate = new ChildVerse();
            $this->setVerse($delegate);
        }
